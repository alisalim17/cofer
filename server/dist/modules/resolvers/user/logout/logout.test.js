"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const TestClient_1 = require("../../../test-utils/TestClient");
const createRandomUser_1 = require("../../../test-utils/createRandomUser");
const constants_1 = require("../../../../utils/constants");
test("logging out a user", () => __awaiter(void 0, void 0, void 0, function* () {
    const user = createRandomUser_1.createRandomUser();
    const client = new TestClient_1.TestClient();
    const { cookie } = yield client.register(user);
    const { data } = yield client.me(cookie);
    expect(data).toBeDefined();
    expect(data).toMatchObject({ username: user.username });
    const { cookie: logoutCookie } = yield client.logout();
    expect(logoutCookie[0].split(";")[0]).toEqual(`${constants_1.COOKIE_NAME}=`);
    const { data: data2 } = yield client.me();
    expect(data2).toEqual(null);
}));
//# sourceMappingURL=logout.test.js.map